...
aws lambda update-function-configuration \
    ...
    --memory-size 512 \
    --timeout 30 \
    --environment "Variables={OPENAI_API_KEY=$OPENAI_API_KEY,OPENAI_MODEL=gpt-4o-mini,S3_BUCKET=$S3_BUCKET,S3_PREFIX=logs/,POLICY_INDEX_KEY=policy_index.json,DDB_TICKETS_TABLE=$DDB_TABLE,SIMILARITY_THRESHOLD=0.45}" \
    --region "$REGION" >/dev/null
